...
> **Note:**  
> This replicated repo assumes paths to:
> - Filtered data → already available from the previous repo (`explainable-medical-coding`)
> - Raw data → already available from the previous repo (`explainable-medical-coding`)

### Scope
Used to train **LAAT, CAML, MultiResCNN, Bi-GRU, and CNN**  
(PLM-ICD trained separately with multi-GPU in previous repo).  
Data filtered to LLM-processed `note_id`s.

### Workflow
1. Run:
   ```bash
   python prepare_data/prepare_mimiciv.py
→ generates full mimiciv_icd10.feather

2. Run notebook `files/data/mimiciv_icd10/generate_targets.ipynb` (uses the filtered parquet file from previous repo as well as the generated full mimiciv_icd10.feather from first step)
→ creates 3 filtered datasets under:
  - filtered_targets/mimic_codes_y/
  - filtered_targets/llm_validated_y/
  - filtered_targets/llm_accurate_y/

3. Copy the desired (for the label set you want to train/test your model) filtered `mimiciv_icd10`.feather file to: "files/data/mimiciv_icd10/mimiciv_icd10.feather"
→ This ensures that main.py will use the target label set copied
→ To train another target label set, copy it from the filtered_targets and replace the current one in mimiciv_icd10 folder and re-run the training pipeline

### Changed Files
| File | Change |
|------|---------|
| `configs/data/mimiciv_icd10.yaml` | Removed procedure codes (diagnosis-only setup) |

### Original Repo ReadMe File Content below
