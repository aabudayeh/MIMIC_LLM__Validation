...
# ========== Changed Code ==========
def load_filtered_mimiciv(parquet_path, combined_json_path, save_path):
    with open(combined_json_path) as f:
        data = json.load(f)

    get_codes = lambda items: [x["code"].upper() for x in items if x.get("code")]
    raw = {
        "mimic_codes_y": {d["note_id"]: get_codes(d.get("mimic_codes", [])) for d in data},
        "llm_validated_y": {d["note_id"]: get_codes(d.get("llm_validated_codes", [])) for d in data},
        "llm_accurate_y": {d["note_id"]: get_codes(d.get("llm_accurate_codes", [])) for d in data},
    }

    # keep only mappings where the list is non-empty
    maps = {k: {nid: vals for nid, vals in m.items() if vals} for k, m in raw.items()}

    # intersection of note_ids that have non-empty lists in all three maps
    valid_ids = set(maps["mimic_codes_y"]) & set(maps["llm_validated_y"]) & set(maps["llm_accurate_y"])

    df = pl.read_parquet(parquet_path).with_columns(pl.col("note_id").cast(pl.Utf8))
    df = df.filter(pl.col("note_id").is_in(list(valid_ids)))

    # join the (already filtered) mappings
    for col, mapping in maps.items():
        mdf = pl.DataFrame({"note_id": list(mapping.keys()), col: list(mapping.values())}).with_columns(pl.col("note_id").cast(pl.Utf8))
        df = df.join(mdf, on="note_id", how="left")
