...
> **Note:**  
> This replicated repo runs inside a parent project with:
> - `1-llm-label-refinement/` → produces `processed_data/combined.json`
> 
> Paths assume the current working directory is this repo and reference:
> ```
> ../1-llm-label-refinement/processed_data/combined.json
> ```

### Scope
Used to train/test **PLM-ICD** with **multi-GPU support**, reducing runtime (~2 h vs >10 h).  
Dataset is filtered to only LLM-processed `note_id`s.

### How to Use
1. Prepare data as in the original repo (no manual edits).  
2. To switch label sets, edit  
   `explainable_medical_coding/config/data/mimiciv_icd10.yaml` →  
   set `target_column_name` to:
   - `mimic_codes_y` (original MIMIC/OM)  
   - `llm_validated_y` (LLM-validated/EV)  
   - `llm_accurate_y` (LLM-replaced/ER)

### Changed Files
| File | Change |
|------|---------|
| `train_plm.py` | Added **multi-GPU** training (`# Changed Code`) |
| `config/data/mimiciv_icd10.yaml` | Switched target column; removed procedure codes |
| `datasets/mimiciv_icd10.py` | Added LLM target columns |
| `utils/data_helper_functions.py` | Added note-ID filter function |
| `data/make_mimiciv_icd10.py` | Applied note-ID filter function using `combined.json` |
